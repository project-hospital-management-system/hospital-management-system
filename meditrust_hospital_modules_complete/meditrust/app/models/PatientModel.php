...
class PatientModel {
  ...
  public function __construct(string $dataDir){ $this->store=new JsonStore($dataDir."/patients.json"); }
  public function all(): array { return $this->store->all(); }
  public function create(array $payload): array {
    $items=$this->store->all();
    $id=Utils::uid("P"); while($this->findById($id)) $id=Utils::uid("P");
  ...
  public function findById(string $id): ?array {
    foreach($this->store->all() as $it) if(($it["id"]??"")===$id) return $it;
    return null;
  }
}
